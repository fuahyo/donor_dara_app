...
class UddJadwalDonor extends StatefulWidget {
  const UddJadwalDonor({super.key});

  @override
  State<UddJadwalDonor> createState() => _UddJadwalDonorState();
}

class _UddJadwalDonorState extends State<UddJadwalDonor> {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: BlocConsumer<UnitUddCubit, UnitUddState>(
            listener: (context, uddState) {
          if (uddState is UnitUddIsError) {
            Commons().showSnackBarErrorr(context, uddState.message!);
          }
        }, builder: (context, uddState) {
          if (uddState is UnitUddIsError) {
            return const Center(
              child: LinearProgressIndicator(),
            );
          } else if (uddState is UnitUddIsSuccess) {}
          {
            return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Padding(
                      padding: EdgeInsets.fromLTRB(24, 16, 152, 0),
                      child: Text(
...
class DaftarUDD extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<UnitUddCubit, UnitUddState>(
        builder: (context, uddState) {
      return Container(
